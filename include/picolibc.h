...
/*
 * Static picolibc configuration for the `picolibc` crate.
 *
 * Upstream picolibc normally generates this header from meson/cmake options.
 * Since this crate builds picolibc with cc-rs (no meson), we ship a frozen
 * configuration tuned for freestanding / baremetal targets such as UEFI:
 *
 *   - single threaded, no TLS and no locking
 *   - a single global `errno`
 *   - tiny stdio (the only stdio picolibc ships)
 *   - no semihosting
 *   - no built-in malloc: allocation is delegated to symbols the embedder
 *     provides (e.g. a Rust global-allocator shim, see the crate README)
 *   - IEEE libm (no errno side effects from math)
 *
 * The values must stay in sync with the picolibc version checked out under
