...
//! Lists of the picolibc C source files that make up `libc` and `libm`.
//!
//! Paths are relative to `third_party/picolibc/libc` and
//! `third_party/picolibc/libm` respectively. The lists are derived from
...
// ---------------------------------------------------------------------------
//
// picolibc's aarch64 port supplies optimised mem*/str* routines under
// `libc/machine/aarch64`. Each routine comes as a pair: a `*.S` Neon assembly
// file and a `*-stub.c` C fallback, gated on the same condition
// (`__OPTIMIZE_SIZE__ || __PREFER_SIZE_OVER_SPEED || !__LP64__ || !__ARM_NEON`)
// so exactly one of the two is non-empty in any given build. The asm uses the
// AAPCS64 calling convention (identical for ELF and Windows/UEFI) but ELF-only
// assembler directives, so it only assembles for ELF targets; for COFF/UEFI we
// disable Neon (see build.rs) which flips every pair to its C stub.
//
// `interrupt.c` and `tls.c` are plain C and compile everywhere; `setjmp.S` and
// `interrupt_vector.S` are asm with no C fallback (ELF only).

/// Plain-C aarch64 machine sources + the C fallbacks, relative to `libc/`. Used
/// on every aarch64 target. On ELF (Neon on) the `*-stub.c` files compile empty
/// and the matching `*.S` provide the symbols; on COFF (Neon off) the stubs
/// provide them.
pub(crate) const LIBC_FILES_AARCH64_C: &[&str] = &[
    "machine/aarch64/interrupt.c",
    "machine/aarch64/tls.c",
    "machine/aarch64/memchr-stub.c",
    "machine/aarch64/memcmp-stub.c",
    "machine/aarch64/memcpy-stub.c",
    "machine/aarch64/memmove-stub.c",
    "machine/aarch64/memrchr-stub.c",
    "machine/aarch64/memset-stub.c",
    "machine/aarch64/rawmemchr-stub.c",
    "machine/aarch64/stpcpy-stub.c",
    "machine/aarch64/strchr-stub.c",
    "machine/aarch64/strchrnul-stub.c",
    "machine/aarch64/strcmp-stub.c",
    "machine/aarch64/strcpy-stub.c",
    "machine/aarch64/strlen-stub.c",
    "machine/aarch64/strncmp-stub.c",
    "machine/aarch64/strnlen-stub.c",
    "machine/aarch64/strrchr-stub.c",
];

/// AArch64 Neon assembly sources, relative to `libc/`. ELF targets only.
pub(crate) const LIBC_FILES_AARCH64_ASM: &[&str] = &[
...
/// Generic `libc/` sources that the aarch64 machine port replaces; they must be
/// dropped from `LIBC_FILES` on aarch64 to avoid duplicate symbols. (The other
/// machine routines — memchr, memcmp, memmove, strchr, strlen — have no generic
/// counterpart in `LIBC_FILES`.)
pub(crate) const LIBC_FILES_AARCH64_EXCLUDE: &[&str] = &[
    "string/memcpy.c",
    "string/memrchr.c",
    "string/memset.c",
    "string/rawmemchr.c",
